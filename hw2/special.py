...
model = Seq2seq(feature_dim, vocab_size, 500, max_frame_len, max_sent_len, load_model_path='./models/special/finish')
model.summary()
...
test_special_X, test_special_ids = [], []
for idx, _id in enumerate(test_video_ids):
    if _id in special_ids:
        test_special_X.append(test_X[idx])
        test_special_ids.append(_id)

preds = model.predict(test_special_X)
preds = model.visual(preds, id2word)
with open(fpath_test_output, 'w') as o:
    for sent, video_id in zip(preds, test_special_ids):
